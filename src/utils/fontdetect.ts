...
const fontdetect = (fontFamily: string) => {
    ...
    let getFontData = function (fontFamily: string) {
        // 清除
        ...
        return [].slice.call(data).filter((value: number) => {
            return value != 0;
        });
    };
...
export const getLocalFontlist = () => {
    const fontFiles: Array<{ [key: string]: string }> = [];
    for (let key in dataFont) {
        let item = dataFont[key];
        item.forEach((element: { zh: string; en: string; }) => {
            let { en } = element;
            let result = fontdetect(en);
            if (result) {
                element["type"] = FontFileType.local;
                fontFiles.push(element);
            }
        });
    }
    storage.set("font", fontFiles);
    return fontFiles;
}
